...
++i;                    return msgsrvcanMove(
    ...
    long AutoActor::constructorAuto (byte creatorId 
...

		{
			...
			if ((_ref_asked||_ref_firstStep)) {
				{
					...
					(temp0=_ref_infra, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 183"), temp0)->_msg_isAllowed(myID, _ref_line, _ref_block, _ref_id, true);
					(_ref_asked=false);
					if ((!_ref_shouldStop)) {
						{
			...
			(temp1=_ref_self, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 209"), temp1)->_msg_live(myID);
		}
