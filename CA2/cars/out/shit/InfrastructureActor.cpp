...
++index;            enqueue(paramQueue, paramsValues, maxParamLength);
	...
	byte** InfrastructureActor::getTable() {
...

		{
			if (_ref_isAuto) {
				{
					(_ref_autoCars[(arrayIndexChecker=_ref_id, assertion(arrayIndexChecker >= 0 && arrayIndexChecker <10, string("Array index out of bound: ") + to_string((long long)arrayIndexChecker) + ", method \"" + reactiveClassName + "." + methodName + "\" line 109") , arrayIndexChecker)]=(dynamic_cast<AutoActor*>(_ref_sender)));
					(temp0=_ref_ctrl, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 110"), temp0)->_msg_isAllowed(myID, _ref_line, _ref_block, _ref_id, true);
				}
}
			if ((!_ref_isAuto)) {
				{
					(_ref_mannedCars[(arrayIndexChecker=_ref_id, assertion(arrayIndexChecker >= 0 && arrayIndexChecker <10, string("Array index out of bound: ") + to_string((long long)arrayIndexChecker) + ", method \"" + reactiveClassName + "." + methodName + "\" line 114") , arrayIndexChecker)]=(dynamic_cast<MannedActor*>(_ref_sender)));
					(temp1=_ref_ctrl, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 115"), temp1)->_msg_isAllowed(myID, _ref_line, _ref_block, _ref_id, false);
				}
}
		}
		return 0;
