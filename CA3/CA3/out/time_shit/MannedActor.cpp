			...
			if ((_ref_asked||_ref_firstStep)) {
				{
					...
					(temp0=_ref_infra, assertion(temp0!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 284"), temp0)->_timed_msg_isAllowed(myID, _ref_line, _ref_block, _ref_id, false, _ref_now, 4 + _ref_now);
				}
}
			(temp1=_ref_self, assertion(temp1!= null, "Null Pointer Exception in method " + reactiveClassName + "." + methodName + "line 286"), temp1)->_timed_msg_live(myID, _ref_now, 1 + _ref_now);
		}
...
++index;            enqueue(paramQueue, paramsValues, maxParamLength);
	...
	vector<TimeFrame>** MannedActor::hashtableTimeExtension = (vector<TimeFrame>**)0xFFFFFFFF;
	
	vector<TimeFrame>** MannedActor::getHashtableTimeExtension() {
	    ...
	    void MannedActor::_timed_msg_canMove (byte senderId
	        ,
    		boolean
	         _ref_canMove
	        , 
...
, TIME_TYPE executionTime, TIME_TYPE deadline){
	    ...
	    void MannedActor::_timed_msg_live (byte senderId
, TIME_TYPE executionTime, TIME_TYPE deadline){
	    _msg_live (senderId
);
	    addTimedBundles(senderId, executionTime, deadline);
		}
	    void MannedActor::_timed_msg_randomMove (byte senderId
, TIME_TYPE executionTime, TIME_TYPE deadline){
	    _msg_randomMove (senderId
);
	    addTimedBundles(senderId, executionTime, deadline);
		}
	void MannedActor::setNow(TIME_TYPE now) {
		...
		{
				__res = _ref_now + (__tNumber|=1, nonDetVariable0);
	__pc = 0;
	shift = 0;
		if (__tNumber & 1) {
